...
-- 2026-03-20: Ledger refactor (consumption uses ingredient_id)
-- Update mark_meal_eaten so consumption debits read and write the ledger
-- using ingredient_id rather than case-insensitive ingredient_name matches.
...
begin
  ...
  loop
    ...
    -- Current stock from ledger (inventory_stock_levels) via ingredient_id
    select coalesce(isl.current_quantity, 0)
      into v_current_qty
    from public.inventory_stock_levels isl
    where isl.user_id = p_user_id
      and isl.ingredient_id = v_ingredient.ingredient_id
